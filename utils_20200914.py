...
def get_f_name(DATA, SF, CM, OP, VS, NotInclude):
    F_NAME = []
    for root,dirs,files in os.walk(DATA):
        for file in files:
            temp = os.path.splitext(file)
            Include = temp[0]+temp[1]
            if Include not in NotInclude:
                F_NAME.append(DATA+Include)
            else:
                pass
    return F_NAME
...
def get_binary_f_dict(F_NAME):
    ...
    for f_name in F_NAME:
        with open(f_name) as inf:
            for line in inf:
                g_info = json.loads(line.strip())
                binary_f = g_info['src'].split('/')[0] + ":" + \
                           g_info['fname']
                if (binary_f not in name_dict):
                    name_dict[binary_f] = name_num
                    name_num += 1
    return name_dict
...
def get_auc_epoch(model, graphs, classes, batch_size,logdir, load_data=None):
    tot_diff = []
    tot_truth = []
    ...
    similar_rate = (1-diff)/2

    DrawPic.Draw_ROC_K(similar_rate,truth,logdir)


    model_auc,fpr, tpr, thres = DrawPic.DrawROC(truth,(1-diff)/2,logdir)
    DrawPic.DrawRecall_Pre_F1(truth, (1-diff)/2, logdir)
